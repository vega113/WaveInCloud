...
public class WaveServerTest extends TestCase {
  ...
  private List<CoreWaveletOperation> addParticipantToWavelet(ParticipantId user) {
    return Arrays.<CoreWaveletOperation>asList(new CoreAddParticipant(user));
  }

  private void submitDeltaToNewWavelet(
      WaveletName name, ParticipantId user, List<CoreWaveletOperation> ops) {
    HashedVersion version = versionZeroFactory.createVersionZero(name);

    CoreWaveletDelta delta = new CoreWaveletDelta(user, version, ops);
