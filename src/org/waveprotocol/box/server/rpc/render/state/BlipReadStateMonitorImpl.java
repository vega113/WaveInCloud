...
public final class BlipReadStateMonitorImpl extends ObservableSupplementedWave.ListenerImpl
   ...
   */
  private static final Logger LOG = Logger.getLogger(BlipReadStateMonitorImpl.class.getName());
  ...
  private void notifyListeners() {
    LOG.fine(Joiner.on(",").join(waveId, ": notifying read/unread change ", read, "/", unread));
  ...
  private void logChange(String action, ConversationBlip blip) {
    LOG.fine(Joiner.on(",").join(blip, ": ", action, " now ", getReadCount(), "/", getUnreadCount()));
  }
