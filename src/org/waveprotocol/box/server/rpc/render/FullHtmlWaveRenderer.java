...
import org.waveprotocol.box.server.rpc.render.view.builder.ParticipantNameViewBuilder;
...
public final class FullHtmlWaveRenderer implements RenderingRules<UiBuilder> {
  ...
  public interface DocRefRenderer {
    ...
    public static final DocRefRenderer EMPTY = new DocRefRenderer() {
      ...
      public UiBuilder render(ConversationBlip blip,
          IdentityMap<ConversationThread, UiBuilder> replies) {
        return UiBuilder.Constant.of(EscapeUtils.fromSafeConstant(""));
      }
    };
  }
  ...
  public FullHtmlWaveRenderer(ShallowBlipRenderer blipPopulator, DocRefRenderer docRenderer,
      ProfileManager profileManager, ViewIdMapper viewIdMapper, ViewFactory viewFactory,
      ThreadReadStateMonitor readMonitor, WavePanelResources resources, String waveletUri) {
  ...
  public UiBuilder render(Conversation conversation, ParticipantId participant) {
    ...
    ParticipantNameViewBuilder participantUi = null;
    participantUi =
      ParticipantNameViewBuilder.create(resources, id);
  ...
  public UiBuilder render(final ConversationThread thread,
    ...
    if (thread.getConversation().getRootThread() == thread) {
      ReplyBoxViewBuilder replyBoxBuilder =
          ReplyBoxViewBuilder.create(resources, replyIndicatorId);
      ...
      ContinuationIndicatorViewBuilder indicatorBuilder = ContinuationIndicatorViewBuilder.create(
          resources, replyIndicatorId);
      InlineThreadViewBuilder inlineBuilder =
          InlineThreadViewBuilder.create(resources, threadId, blipsUi, indicatorBuilder);
  ...
  public UiBuilder render(final ConversationBlip blip, UiBuilder document,
    ...
    BlipMetaViewBuilder metaUi = null;
    metaUi = BlipMetaViewBuilder.create(resources, viewIdMapper.metaOf(blip), document);
    metaUi.setBlipUri(waveUri + "/" + blip.hackGetRaw().getId());
    blipPopulator.render(blip, metaUi);
    return BlipViewBuilder.create(resources, viewIdMapper.blipOf(blip), metaUi, threadsUi, convsUi);
  }
