...
public final class BlipMetaViewBuilder implements UiBuilder, IntrinsicBlipMetaView {
   ...
   *        characters
   */
  public static BlipMetaViewBuilder create(WavePanelResources resources, String id,
      UiBuilder content) {
    return new BlipMetaViewBuilder(resources.getBlip().css(), id, nonNull(content));
  }
  ...
  public void outputHtml(SafeHtmlBuilder output) {
    ...
    open(output, id, css.meta(), TypeCodes.kind(Type.META));
    {
          ...
          css.metabar() + " " + (read ? css.read() : css.unread()), null);
      {
        ...
        menuBuilder(options, selected, css, blipUri).outputHtml(output);
        close(output);
  ...
  public static UiBuilder menuBuilder(final Set<MenuOption> options, final Set<MenuOption> selected,
      final BlipViewBuilder.Css css, final String blipUri) {
...
//          out.append(EscapeUtils.fromSafeConstant("|"));
        ...
        out.append(EscapeUtils.fromSafeConstant("<a href='" + blipUri + "' target='_blank'>Log in to edit</a>"));
//        out.append(EscapeUtils.fromSafeConstant("|"));
//        openSpanWith(out, null, css.menuOption(), TypeCodes.kind(Type.MENU_ITEM), "{{link}}");
//        out.append(MENU_LABELS.get(MenuOption.LINK));
//        closeSpan(out);
      }
    };
  }
