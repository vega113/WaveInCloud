...
public class XmppFederationRemote implements WaveletFederationProvider {
  ...
  public void submitRequest(final WaveletName waveletName,
    ...
    try {
      ...
      listener.onFailure(FederationError.newBuilder().setErrorCode(
          FederationError.Code.BAD_REQUEST).setErrorMessage(
          "couldn't encode wavelet name " + waveletName).build());
      return;
    }
    ...
    disco.discoverRemoteJid(waveletName.waveletId.getDomain(),
        new SuccessFailCallback<String, String>() {
          ...
          public void onFailure(String errorMessage) {
            ...
            listener.onFailure(FederationErrors.newFederationError(
                FederationError.Code.BAD_REQUEST,
  ...
  public void update(final Message updateMessage, final PacketCallback responseCallback) {
    ...
    if (items == null) {
      responseCallback.error(
          FederationErrors.badRequest("Items element missing from update message"));
      return;
    }
    ...
    for (Element item : elements) {
      ...
      if (waveletUpdate == null) {
        callback.onFailure(FederationErrors.badRequest("wavelet-update element "
            + "missing from message: " + updateMessage));
        continue;
      }
      ...
      try {
        ...
        callback.onFailure(FederationErrors.badRequest("Couldn't decode wavelet name: "
      ...
      if (!deltas.isEmpty()) {
        ...
        listener.waveletDeltaUpdate(waveletName, builder.build(), callback);
      }
   ...
   */
  private void processSubmitResponse(Packet result,
    ...
    if (pubsub == null || publish == null || item == null
      ...
      listener.onFailure(FederationError.newBuilder().setErrorCode(
          FederationError.Code.BAD_REQUEST).setErrorMessage(
          "invalid submitResponse: " + result).build());
      return;
    }

    ProtocolHashedVersion.Builder hashedVersion =
        ProtocolHashedVersion.newBuilder();
    hashedVersion.setHistoryHash(Base64Util.decode(hashedVersionElement
        .attributeValue("history-hash")));
    hashedVersion.setVersion(Long.parseLong(hashedVersionElement
        .attributeValue("version")));
    long applicationTimestamp =
        Long.parseLong(submitResponse
            .attributeValue("application-timestamp"));
    int operationsApplied =
        Integer.parseInt(submitResponse
            .attributeValue("operations-applied"));
  ...
  private void processHistoryResponse(Packet historyResponse,
      ...
      for (Element itemElement : (List<Element>) items.elements()) {
        for (Element element : (List<Element>) itemElement.elements()) {
          ...
          if (elementName.equals("applied-delta")) {
            ...
            deltaList.add(ByteString.copyFrom(Base64.decodeBase64(deltaBody
                .getBytes())));
            ...
            listener.onFailure(FederationError.newBuilder().setErrorCode(
                FederationError.Code.BAD_REQUEST).setErrorMessage(
                "bad response packet: " + historyResponse).build());
          }
        }
      }
    ...
    if (lastCommittedVersion > -1) {
      ...
      lastCommitted = ProtocolHashedVersion.newBuilder()
          .setVersion(lastCommittedVersion).setHistoryHash(ByteString.EMPTY).build();
    } else {
      lastCommitted = null;
    }
   ...
   */
  private void processGetSignerResponse(Packet packet,
    ...
    if (signature == null) {
      ...
      listener.onFailure(FederationErrors
          .badRequest("bad getDeltaSignatureRequest response"));
      return;
    }
